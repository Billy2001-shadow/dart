    ...
    def __init__(self, filelist_path, mode, size=(518, 518)):      
        ...
        self.transform = Compose([
            Resize(
                ...
                resize_method='lower_bound',                            # 至少最短边达到目标尺寸的net_w或net_h，keep_aspect_ratio为True保持
...
def get_kitti_loader(data_dir_root,mode, size=(448, 448)):
    dataset = KITTI(data_dir_root, mode,size)
    if mode  == "train":
        return DataLoader(dataset, batch_size=14, shuffle=False,num_workers=16,pin_memory=True)
    elif mode == 'val':
